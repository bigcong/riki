...
def add_layer(inputs, in_size, out_size, activation_function=None, n_layer=None):
    ...
    with tf.name_scope(layer_name):
        with tf.name_scope('weights'):
            ...
            tf.summary.histogram(layer_name + '/weights', Weight)  # tensorflow >= 0.12

        # 定一个矩阵1*out_size 都是0.1 的矩阵
        with tf.name_scope('biases'):
            Biase = tf.Variable(tf.zeros([1, out_size]) + 0.1)
            tf.summary.histogram(layer_name + '/biases', Biase)  # Tensorflow >= 0.12

    with tf.name_scope('yuche'):
...
ll = add_layer(xs, 1, 10, activation_function=tf.nn.relu, n_layer=1)

prediction = add_layer(ll, 10, 1, activation_function=None, n_layer=2)
